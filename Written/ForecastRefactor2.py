...
def recursiveChange (method_return, current_val, current_data, region_size, region_index ):
  if bound_split and recursive_split:
  ...
  return DataMod.genChange(method_return)
...
for index1 in np.arange(num_forecasts):
  ...
  for index2 in np.arange(1, Forecast_iterations):
    ...
    current_val +=  recursiveChange(method_return, current_val, current_data, region_size, region_index)
