...
moving_avg = Forecast.rolling(window=10).mean()
moving_distr = Forecast.apply(DataMod.blitzDistr, axis = 0, return_type = 'conc range')
max_mvDistri = moving_distr.max().max()
min_mvDistri = moving_distr.min().min()

from_ = data.size
x_range = np.arange(from_, from_ + Forecast.shape[1])

plt.plot(data, color = 'orange')
plt.plot(x_range, Forecast.T, color = 'grey', alpha = 0.2)
