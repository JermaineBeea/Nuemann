...
# Function to calculate distribution of data
def distribution(data, deviation_type, output, tend_evaluation = True):
  data = np.array(data)
  # Calculate deviation and tendency based on the deviation_type and tend_evaluation
  if tend_evaluation:
  ...
  # Calculate distribution and range
  ...
  # Check if output is in return library
...
# Function to calculate distribution of data in a faster way
def blitzDistr(data, deviation_type, output, tend_evaluation = True):
    data_array = np.array(np.concatenate(data))[:, np.newaxis] 
    # Calculate deviation, tendency, distribution and range based on the deviation_type and tend_evaluation
    if tend_evaluation:
...
# Function to calculate probability distribution of data
def probDistr(distr_range, original_data, return_type = (1,2,3,4,5), decimal_convert = True, roundup = None):
  ...
  # Convert original_data to numpy array if it's not
  if not isinstance(original_data, np.ndarray):
    original_data = np.array(original_data)
  # Initialize CounterArr and count
  CounterArr = np.array([]); 
  Datasize = original_data.size; count = 0
  # Calculate CounterArr
  ...
  # Calculate SumCount, ProbArr, NetProb and RelativeProb
...
def blitzProbDistr(distr_range, original_data, return_type = (1,2,3,4,5), decimal_convert = True, roundup = None):
  ...
  # Calculate Datasize, ranges, in_range, CounterArr, SumCount, ProbArr, NetProb and RelativeProb
...
# Function to filter and pair data
...
# Function to calculate differences of data
def diffFunction(data, n_order = 1, return_type = 0):
  ...
  # Check the shape of data and calculate differences
  if len(data.shape) < 2:   
  ...
  # Calculate diffsize, Posvalues, Negvalues, Posprob and NegProb
...
# Function to splice index of data
